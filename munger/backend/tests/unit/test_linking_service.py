...
class TestLinkingServiceScoring:
    def test_hybrid_score_exact_name(self):
        ...
        a = Entity(id=1, name="Charlie Munger", entity_type="person", embedding=[1.0] + [0.0] * 767)
        b = Entity(id=2, name="Charlie Munger", entity_type="person", embedding=[1.0] + [0.0] * 767)
        assert svc._hybrid_score(a, b) >= svc.settings.link_auto_merge

    def test_hybrid_score_different_entities_low(self):
        svc = LinkingService(settings=Settings())
        a = Entity(id=1, name="System 1", entity_type="concept", embedding=[1.0] + [0.0] * 767)
        b = Entity(id=2, name="System 2", entity_type="concept", embedding=[0.0] * 768)
        assert svc._hybrid_score(a, b) < svc.settings.link_auto_merge
